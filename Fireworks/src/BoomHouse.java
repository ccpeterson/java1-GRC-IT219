...
public class BoomHouse extends GraphicsProgram
{
	final int WIDTH = 800;
	...
	{
		//Draw Sign
		GLabel sign = new GLabel("Boom-House!",140,290);
	...
	public void drawShow()
	{
		...
		int end2Y = 100;
		int boomSize = 100;
		drawFireLine(baseX,baseY,end1X,end1Y);
		drawBoom(end1X,end1Y,boomSize);
		drawFireLine(baseX,baseY,end2X,end2Y);		
		drawBoom(end2X,end2Y,boomSize);
	}
	
	public void drawFireLine(double startX, double startY, double endX, double endY)
	{
		double moveX = startX + .1*(endX - startX);
		double moveY = startY - .1*(startY - endY);
		GLine fireLine = new GLine(startX,startY,moveX,moveY);
		fireLine.setColor(Color.ORANGE);
		add(fireLine);
		for (int i = 1; i <= 9; i++)
		{
			pause(150);
			moveX = moveX + (.1*(endX - startX));
			moveY = moveY - (.1*(startY - endY));
			fireLine.setEndPoint(moveX, moveY);			
		}
		pause(150);		
		fireLine.scale(0);
	}
	public void drawBoom(int boomX, int boomY, int boomSize)
	{
		//Draw boom
		
		GOval boom = new GOval(boomX, boomY, boomSize, boomSize);
		boom.setFilled(true);
